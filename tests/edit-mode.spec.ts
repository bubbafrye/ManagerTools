...
test("number and font controls update CSS tokens live", async ({ page }) => {
  ...
  await page.getByLabel("borders").fill("5");
  ...
  const tokens = await page.evaluate(() => {
    ...
    return {
      ...
      card2: root.getPropertyValue("--containers-card2-radii").trim(),
  ...
  expect(tokens.card2).toBe("8px");
  expect(tokens.card1Stroke).toBe("4px");
  expect(tokens.card2Stroke).toBe("4px");
...
test("show completed toggle is in edit mode next to Add Item", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Document settings" }).click();
  await expect(
    page.locator("[data-layout='theme-editor']").getByRole("checkbox", {
      name: "show completed",
    }),
  ).toHaveCount(0);
...
test("edit mode shows look-editor swatches in the panel, not section headers", async ({
  ...
  await expect(page.getByLabel("panels surface")).toHaveCount(0);
  await page.getByRole("button", { name: "Edit theme" }).click();
  const panel = page.locator("[data-layout='theme-config']");
...
test("themes sit on the left, edit button on the right, and rando still randomizes", async ({
  ...
  expect(after.panelStroke).toBe(Math.round(after.containerStroke * 0.8));
  expect(after.containerRadius).toBeGreaterThanOrEqual(0);
  expect(after.containerRadius).toBeLessThanOrEqual(30);
  expect(after.containerStroke).toBeGreaterThanOrEqual(0);
  expect(after.containerStroke).toBeLessThanOrEqual(10);
  expect(after.panelStroke).toBeGreaterThanOrEqual(0);
  expect(after.panelStroke).toBeLessThanOrEqual(8);
  expect(after.uiStroke).toBeGreaterThanOrEqual(0);
  expect(after.uiStroke).toBeLessThanOrEqual(10);
...
test("theme presets apply Figma mode tokens without changing fonts", async ({
  ...
  await page.getByRole("button", { name: "header font" }).click();
  await page.getByRole("option", { name: "Lora" }).click();
  await page.getByLabel("Theme name").fill("Keep fonts");
...
test("save theme commits a named swatch to the document", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto("/");
  await page.getByRole("button", { name: "Document settings" }).click();
  ...
  await dialog.getByLabel("Theme name").fill("My look");
  await dialog.getByRole("button", { name: "Save theme" }).click();
  await expect(dialog).toHaveCount(0);
  await expect(page.getByRole("button", { name: "My look" })).toBeVisible();
});
